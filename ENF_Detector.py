...
def main_code(aud_file, enf_fre):
    """
    Function for ENF line detection and save the result in JSON format.
    aud_file:  path to the audio file
    enf_fre:  ENF to be detected
    return: Save result in JSON format
    """
    currentPath = os.path.dirname(os.path.realpath(__file__))
    outputPath = os.path.join(currentPath, 'tmp')

    # variable to assign ENF line length
    enf_len = 0

    # loading audio file
    fs1, audio1 = wav.read(aud_file)
    ...
    if len(contours) != 0:
        ...
        print("bounding box length", w)
        print("bounding box x value", x)
    ...
    if enf_fre == "60":
        ...
        # condition for tempering detection
        if 40 > y > 20 and w < line_len:
            tempered = "Audio File is Tempered"

        else:
            tempered = "Tempering not detected"
    if result == "ENF Detected":
        xy = (duration_seconds / width)
        print("x", x)
        print("start", xy * x)
        print("end", xy * (x + w))
    else:
        print("ENF not detected")
